...
We can get the permutation importance for the SVM model:

```{r}

features <- downsampled_train_data[, -which(names(downsampled_train_data) == "label")]

# Create a grid for the features
xrange <- seq(min(features[,1]) - 1, max(features[,1]) + 1, by = 0.05)
yrange <- seq(min(features[,2]) - 1, max(features[,2]) + 1, by = 0.05)
grid <- expand.grid(x = xrange, y = yrange)

# Predict probabilities for the grid points
grid$prob <- predict(svm_model, grid, probability = TRUE, decision.values = TRUE)

# Extract the decision values for plotting decision boundaries
grid$decision <- attr(grid$prob, "decision.values")

# Plotting the decision boundaries
plot <- ggplot() +
  geom_point(data = grid, aes(x = x, y = y, color = decision)) +
  geom_point(data = downsampled_train_data, aes(x = downsampled_train_data[,1], y = downsampled_train_data[,2], color = label)) +
